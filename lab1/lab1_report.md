...
После создания VM я подключился через SSH используя `gcloud` для поиска и копирования файлов из бакета `lab1-bucket-itmo` на локальную директорию ВМ.
...
На следующем этапе я измененил права доступа для созданного service account:
...
На начальном этапе работы доступ к файлам в бакете `lab1-bucket-itmo` был успешно получен с помощью утилиты `gcloud storage cp`. Однако при изменении роли service account с `Storage Admin` на `Compute Viewer` операции копирования файлов **продолжали работать успешно**.

Честно говоря я не знаю в чем причина.
